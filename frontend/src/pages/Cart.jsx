...
const cart = () => {
  ...
  const { products, currency, cartItems, updateQuantity, delivery_fee, setCartItems, navigate } = useContext(ShopContext);
      ...
      for(const item in cartItems[items]){
        try {
          if(cartItems[items][item] > 0){
            let product = products.find(product => product.id === items);
            tempData.push({
              _id: items,
              size: item,
      ...
      <div className='flex justify-end my-20'>
        <div className='w-full sm:w-[450px]'>
          ...
          <div className='w-full text-end'>
            <button onClick={()=>navigate('/place-order')} className='bg-black text-white text-sm my-8 px-8 py-3 cursor-pointer'>LANJUTKAN KE PEMBAYARAN </button>
